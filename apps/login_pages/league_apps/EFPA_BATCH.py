    ...
    with col1:
        ...
        if st.checkbox("Process data "):
            ...
            if submit:                                            
                ...
                if flag2 == []:
                    insert_EFPA_LEAGUE_flag_option(flag_option,rememmberr,id)
                    df = to_append
                    ...
                    a = view_all_EFPA__LEAGUE_flag_record(id)
       ...
       # Delite datas 
        ...
        try:
                    ...
                    if int(temp_save) > 0:
                        ...
                        if flag_option !=[]:
                            if temp_filter == 'LEAUGE':
                                ...
                                st.success("Visualization of expend by the player by League")
                                ##      1. Graph 
                                df = pd.read_sql_query('SELECT * FROM EFPA_BATCH_table WHERE user_id = "{}"'.format(temp_save),conn)
                                df_new = df[["Name_of_Legue","Year","Nationality","Expend_by_player","Expend_INFLACION"]]
                                ...
                                #   ---------------------------------------------------------------
                                st.success("Visualization of expend by the player by League with inflation the rate")
                                ...
                                st.success("Visualization of costs per the player according to the selected season")
                                ...
                                st.success("Visualization of costs per the player according to the selected season")
                                error_bars = alt.Chart(df_new).mark_errorbar(extent='ci').encode(
                                  x=alt.X('Expend_INFLACION', scale=alt.Scale(zero=False)),
                                  y=alt.Y('Year'),
                                  color =  'Year'
                                ).properties(
                                    width=600,
                                ...
                                points = alt.Chart(df_new).mark_point(filled=True, color='black',size=90).encode(
                                  x=alt.X('Expend_INFLACION', aggregate='mean'),
                                  y=alt.Y('Year'),
                                  color =  'Year'
                                ).properties(
                                    width=600,
                                ...
                                st.success("Visualization of costs per the player according to the selected season")
                                test = alt.Chart(df_new).mark_circle(size=60).encode(
                                    y=alt.Y('Expend_INFLACION', axis=alt.Axis(title=None)),
                                    x=alt.X('Nationality', axis=alt.Axis(labels=False,title=None)),
                                    color='Year'
                                ).properties(
                                    width=600,
                                ...
                                points = alt.Chart(df_new).mark_point().encode(
                                    x=alt.X('Name_of_Legue', axis=alt.Axis(labels=False,title=None)),
                                    y=alt.Y('Expend_INFLACION', axis=alt.Axis( title='Expend by player')),
                                    color=alt.condition(brush, 'Year', alt.value('lightgray'))
                                ).add_selection(
                                    brush
                                ).properties(
                                ...
                                bars = alt.Chart(df_new).mark_bar().encode(
                                    ...
                                    x=alt.Y('Expend_INFLACION', axis=alt.Axis( title='Expend by player')),
                                ).transform_filter(
                                    brush
                                ).properties(
                                    width=500,
                                ...
                                st.success("Visualization of expend by the player by Nationality")
                                #       Expend by player 
                                #   ---------------------------------------------------------------
                                ##      1. Graph 
                                df = pd.read_sql_query('SELECT * FROM EFPA_BATCH_table WHERE user_id = "{}"'.format(temp_save),conn)
                                ...
                                st.write(points + lines)
                                #   ---------------------------------------------------------------
                                st.success("Visualization of expend by the player by Nationality with inflation the rate")
                                #       Expend by player + INFATION 
                                #   ---------------------------------------------------------------
                                ##      1. Graph 
                                df = pd.read_sql_query('SELECT * FROM EFPA_BATCH_table WHERE user_id = "{}"'.format(temp_save),conn)
          ...
          st.write("Error, please resart Visaulsation checkbox !! ") 
